...
for n in (cm)
  if ($mail_agent:is_recipient(n[1]))
    old = n[1]:length_num_le(n[2]);
    olddate = n[1]:length_date_le(n[3]);
    "Make sure that we don't skip messages sent in the same second, and also handle issue when messages were renumbered by checking both date AND message number.";
    old = min(old, olddate);
    leng_all = n[1]:length_all_msgs();
    if (old < leng_all)
      next = old + 1;
